...
class Dbconnection {
	...
	// public $Database_name = "main_labs";
	...
	function __construct() {
	     ...
	     $this->Database_user = $_ENV["DB_USER"];
	     $this->Database_pass = $_ENV["DB_PWD"];
		...
		if (strpos($_SERVER['HTTP_HOST'], 'nigsoft') !== false) {
			...
			$this->Host_name = $_ENV["DB_RDS_ENDPOINT"];
		}
	...
	function mysql_update($table, $array, $condition) {
		$table = trim($table);
		if ($table == '' || $condition == '') {
			$this->error("Error table or condition not defenide,mysql_update()");
		}
		if (strpos($condition, '=') !== false) {

		} else {
			$this->error("Error condition not defenide,mysql_update()");
		}
