...
if (count($result) > 0) {
	...
	foreach ($result as $row) {
		...
		{
		$out .="<a class='btn btn-sm btn-info' title='Edit' style='padding:2px 0px 2px 7px; text-align:center;' href='editpurchaseorder.php?bill_check_group=".base64_encode($row['orderid'])."'><i class='feather icon-edit'></i></a>&nbsp;<a class='btn btn-sm btn-warning' href='purchase.php?bill_check_group=".base64_encode($row['orderid'])."'>Invoice</a>";
		}
